...
class TestArity extends TestBase {
	override function test() {
		...
		assertError("ReqlCompileError", "Expected between 1 and 2 arguments but found 3.", db.tableDrop(1));
		assertError("ReqlCompileError", "Expected between 2 and 3 arguments but found 1.", r.expr([]).deleteAt());
		assertError("ReqlCompileError", "Expected between 1 and 2 arguments but found 3.", tbl.count(1));
		assertError("ReqlCompileError", "Expected 2 arguments but found 1.", tbl.reduce());
		assertError("ReqlCompileError", "Expected 3 or more arguments but found 2.", r.branch(1));
		assertError("ReqlQueryLogicError", "Cannot call `branch` term with an even number of arguments.", r.branch(1, 3));
		...
		assertError("ReqlQueryLogicError", "Expected 0 arguments but found 3.", r.now(r.args([1, 3])));
	}
}