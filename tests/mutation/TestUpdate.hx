...
@:await class TestUpdate extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", "Durability option `wrong` unrecognized (options are \"hard\" and \"soft\").", [0]), tbl.get(12).update(function(row:Expr):Expr return { "a" : row["id"] + 3 }, { "durability" : "wrong" }));
		@:await assertAtom({ "id" : 12, "a" : 14 }, tbl.get(12));
		...
		@:await assertError(err("ReqlQueryLogicError", "Could not prove argument deterministic.  Maybe you want to use the non_atomic flag?", [0]), tbl.get(1).update({ "d" : r.js("5") }));
		@:await assertError(err("ReqlQueryLogicError", "Could not prove argument deterministic.  Maybe you want to use the non_atomic flag?", [0]), tbl.get(1).update({ "d" : tbl.nth(0) }));
