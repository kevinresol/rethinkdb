...
@:await class TestAtomic_get_set extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", "Error:" + " encountered obsolete optarg `return_vals`.  Use `return_changes` instead.", [0]), tbl.insert({ "id" : 0 }, { "return_vals" : true }).pluck("changes", "first_error"));
		...
		@:await assertAtom(({ "changes" : [{ "old_val" : { "id" : 0, "x" : 1 }, "new_val" : { "id" : 0, "x" : 3 } }, { "old_val" : { "id" : 1 }, "new_val" : { "id" : 1, "x" : 3 } }] }), tbl.update({ "x" : 3 }, { "return_changes" : true }).pluck("changes", "first_error").do_(function(d:Expr):Expr return d.merge({ "changes" : d["changes"].orderBy(function(a:Expr):Expr return a["old_val"]["id"]) })));
		...
		@:await assertAtom(({ "changes" : [], "first_error" : "a" }), tbl.get(0).replace(function(y:Expr):Expr return { "x" : r.error("a") }, { "return_changes" : true }).pluck("changes", "first_error"));
		@:await assertAtom(({ "first_error" : "a", "changes" : [{ "old_val" : { "id" : 0, "x" : 2 }, "new_val" : { "id" : 0, "x" : 2 }, "error" : "a" }] }), tbl.get(0).replace(function(y:Expr):Expr return { "x" : r.error("a") }, { "return_changes" : "always" }).pluck("changes", "first_error"));
		@:await assertAtom(({ "changes" : [{ "new_val" : { "id" : 0 }, "old_val" : { "id" : 0, "x" : 2 } }, { "new_val" : { "id" : 1 }, "old_val" : { "id" : 1, "x" : 3 } }] }), tbl.replace(function(y:Expr):Expr return y.without("x"), { "return_changes" : true }).pluck("changes", "first_error").do_(function(d:Expr):Expr return d.merge({ "changes" : d["changes"].orderBy(function(a:Expr):Expr return a["old_val"]["id"]) })));
		@:await assertAtom(({ "first_error" : "Inserted object must have primary key `id`:\n{\n\t\"x\":\t1\n}", "changes" : [{ "new_val" : { "id" : 0 }, "old_val" : { "id" : 0 }, "error" : "Inserted object must have primary key `id`:\n{\n\t\"x\":\t1\n}" }, { "new_val" : { "id" : 1 }, "old_val" : { "id" : 1 }, "error" : "Inserted object must have primary key `id`:\n{\n\t\"x\":\t1\n}" }] }), tbl.replace({ "x" : 1 }, { "return_changes" : "always" }).pluck("changes", "first_error").do_(function(d:Expr):Expr return d.merge({ "changes" : d["changes"].orderBy(function(a:Expr):Expr return a["old_val"]["id"]) })));
