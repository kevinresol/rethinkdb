...
@:await class TestDelete extends TestBase {
	...
	override function test() {
		...
		@:await assertAtom(({ "deleted" : 0, "replaced" : 0, "unchanged" : 0, "errors" : 0, "skipped" : 0, "inserted" : 100 }), tbl.insert([for (i in xrange(100)) { "id" : i }]));
		...
		@:await assertError(err("ReqlQueryLogicError", "Durability option `wrong` unrecognized (options are \"hard\" and \"soft\").", [0]), tbl.skip(50).delete({ "durability" : "wrong" }));
		...
		@:await assertError(err("ReqlQueryLogicError", "Expected type SELECTION but found DATUM:", [0]), r.expr([1, 2]).delete());
