...
@:await class TestReplace extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", "Could not prove argument deterministic.  Maybe you want to use the non_atomic flag?", [0]), tbl.get(1).replace(r.row.merge({ "c" : r.js("5") })));
		@:await assertError(err("ReqlQueryLogicError", "Could not prove argument deterministic.  Maybe you want to use the non_atomic flag?", [0]), tbl.get(1).replace(r.row.merge({ "c" : tbl.nth(0) })));
