...
@:await class TestInsert extends TestBase {
	...
	override function test() {
		...
		@:await assertPartial({ "tables_created" : 1 }, r.db("test").tableCreate("test2"));
		...
		@:await assertError("ReqlQueryLogicError", "Durability option `wrong` unrecognized (options are \"hard\" and \"soft\").", tbl.insert({ "id" : 3, "a" : 3 }, { "durability" : "wrong" }));
		@:await assertAtom(3, tbl.count());
		...
		@:await assertError("ReqlQueryLogicError", "Conflict option `wrong` unrecognized (options are \"error\", \"replace\" and \"update\").", tbl.insert({ "id" : 3, "a" : 3 }, { "conflict" : "wrong" }));
		...
		@:await assertPartial({ "tables_dropped" : 1 }, r.db("test").tableDrop("testpkey"));
		@:await assertAtom({ "deleted" : 0, "replaced" : 0, "unchanged" : 0, "errors" : 0, "skipped" : 0, "inserted" : 7 }, tbl.forEach(function(row) return tbl2.insert(row.merge({ "id" : row["id"] + 100 }))));
		@:await assertPartial({ "errors" : 1, "first_error" : "`r.minval` and `r.maxval` cannot be written to disk." }, tbl.insert({ "value" : r.minval }));
		@:await assertPartial({ "errors" : 1, "first_error" : "`r.minval` and `r.maxval` cannot be written to disk." }, tbl.insert({ "value" : r.maxval }));
		@:await assertPartial({ "changes" : [for (i in range(1, 100)) { "old_val" : { "id" : 100 + i, "ordered-num" : i }, "new_val" : { "id" : 100 + i, "ordered-num" : i }, "error" : "Duplicate primary key `id`:\n{\n\t\"id\":\t" + str(100 + i) + ",\n\t\"ordered-num\":\t" + str(i) + "\n}\n{\n\t\"id\":\t" + str(100 + i) + ",\n\t\"ordered-num\":\t" + str(i) + "\n}" }] }, tbl.insert([for (i in range(1, 100)) { "id" : 100 + i, "ordered-num" : i }], { "return_changes" : "always" }));
		@:await assertPartial({ "changes" : [] }, tbl.insert([for (i in range(1, 100)) { "id" : 100 + i, "ordered-num" : i }], { "return_changes" : true }));
		@:await assertPartial({ "inserted" : 1 }, tbl.insert({ "id" : 42, "foo" : 1, "bar" : 1 }));
		@:await assertPartial({ "replaced" : 1 }, tbl.insert({ "id" : 42, "foo" : 5, "bar" : 5 }, { "conflict" : function(id, old_row, new_row) return old_row.merge(new_row.pluck("bar")) }));
		@:await assertAtom({ "id" : 42, "foo" : 1, "bar" : 5 }, tbl.get(42));
		@:await assertPartial({ "first_error" : "Inserted value must be an OBJECT (got NUMBER):\n2" }, tbl.insert({ "id" : 42, "foo" : 1, "bar" : 1 }, { "conflict" : function(a, b, c) return 2 }));
		@:await assertError("ReqlQueryLogicError", "The conflict function passed to `insert` should expect 3 arguments.", tbl.insert({ "id" : 42 }, { "conflict" : function(a, b) return a }));
		@:await assertError("ReqlQueryLogicError", "The conflict function passed to `insert` must be deterministic.", tbl.insert({ "id" : 42 }, { "conflict" : function(a, b, c) return tbl.get(42) }));
		@:await assertPartial({ "replaced" : 1 }, tbl.insert({ "id" : 42 }, { "conflict" : function(a, b, c) return { "id" : 42, "num" : "424" } }));
		@:await assertAtom({ "id" : 42, "num" : "424" }, tbl.get(42));
		@:await assertError("ReqlQueryLogicError", "Cannot convert `r.minval` to JSON.", r.minval);
		@:await assertError("ReqlQueryLogicError", "Cannot convert `r.maxval` to JSON.", r.maxval);
		@:await assertPartial({ "tables_dropped" : 1 }, r.db("test").tableDrop("test2"));
