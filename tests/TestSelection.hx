...
@:await class TestSelection extends TestBase {
	...
	override function test() {
		...
		@:await assertAtom(({ "deleted" : 0.0, "replaced" : 0.0, "unchanged" : 0.0, "errors" : 0.0, "skipped" : 0.0, "inserted" : 100 }), tbl.insert(([for (i in xrange(100)) { "id" : i, "a" : i % 4 }] : Array<Dynamic>)));
		@:await assertAtom(({ "deleted" : 0.0, "replaced" : 0.0, "unchanged" : 0.0, "errors" : 0.0, "skipped" : 0.0, "inserted" : 100 }), tbl2.insert(([for (i in xrange(100)) { "id" : i, "b" : i % 4 }] : Array<Dynamic>)));
		@:await assertAtom("TABLE", tbl.typeOf());
		@:await assertError(err("ReqlOpFailedError", "Database `missing` does not exist.", ([0] : Array<Dynamic>)), r.db("missing").table("bar"));
		@:await assertError(err("ReqlOpFailedError", "Table `test.missing` does not exist.", ([0] : Array<Dynamic>)), r.db("test").table("missing"));
		@:await assertAtom(({ "errors" : 1, "inserted" : 0 }), tbl.insert({ "id" : "\x00" }).pluck("errors", "inserted"));
		@:await assertAtom(({ "errors" : 1, "inserted" : 0 }), tbl.insert({ "id" : (["embedded", (["null\x00"] : Array<Dynamic>)] : Array<Dynamic>) }).pluck("errors", "inserted"));
		@:await assertError(err("ReqlQueryLogicError", "Database name `%` invalid (Use A-Z, a-z, 0-9, _ and - only).", ([0] : Array<Dynamic>)), r.db("%"));
		@:await assertError(err("ReqlQueryLogicError", "Table name `%` invalid (Use A-Z, a-z, 0-9, _ and - only).", ([0] : Array<Dynamic>)), r.db("test").table("%"));
		...
		@:await assertAtom(([] : Array<Dynamic>), tbl.getAll());
		@:await assertAtom(([{ "id" : 20, "a" : 0 }] : Array<Dynamic>), tbl.getAll(20));
		...
		@:await assertError(err("ReqlQueryLogicError", "Expected type TABLE_SLICE but found DATUM:", ([0] : Array<Dynamic>)), r.expr(([1, 2, 3] : Array<Dynamic>)).between(-1, 2));
		...
		@:await assertAtom(25, tbl.filter(function(row:Expr):Expr return row["a"] > 2).count());
		...
		@:await assertAtom(([] : Array<Dynamic>), r.expr(([{ "a" : 1 }] : Array<Dynamic>)).filter({ "b" : 1 }));
		@:await assertAtom(25, tbl.count(function(row:Expr):Expr return { "a" : 1 }));
		@:await assertAtom(2, r.expr(([1, 2, 3, 1] : Array<Dynamic>)).count(1));
		@:await assertAtom(2, r.expr(([null, 4, null, "foo"] : Array<Dynamic>)).count(null));
		@:await assertError(err("ReqlQueryLogicError", "Expected type DATUM but found TABLE:", ([0] : Array<Dynamic>)), r.expr(5) + tbl);
