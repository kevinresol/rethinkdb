...
class TestRange extends TestBase {
	override function test() {
		assertAtom([0, 2], r.range().limit(4));
		assertAtom([0, 2], r.range(4));
		assertAtom([2, 4], r.range(2, 5));
		assertAtom([], r.range(0));
		assertAtom([], r.range(5, 2));
		assertAtom([-5, -4, -3], r.range(-5, -2));
		assertAtom([-5, -4, -3, -2, -1, 1], r.range(-5, 2));
		...
		assertAtom(err_regex("ReqlQueryLogicError", "Number not an integer \\(>2\\^53\\). 9007199254740994", []), r.range(9.00719925474099e+15));
		assertAtom(err_regex("ReqlQueryLogicError", "Number not an integer \\(<-2\\^53\\). -9007199254740994", []), r.range(-9.00719925474099e+15));
