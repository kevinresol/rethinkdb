...
@:await class TestPoint extends TestBase {
	...
	override function test() {
		...
		var pluck = tbl.get(3).changes({ "squash" : false, "include_initial" : true }).pluck({ "new_val" : ["red", "blue"] })["new_val"];
		...
		var dtblPluck = dtbl.get(5).changes({ "include_initial" : true }).pluck({ "new_val" : ["red", "blue"] })["new_val"];
