...
@:await class TestObject extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", errmsg, []), r.literal("foo"));
		@:await assertError(err("ReqlQueryLogicError", errmsg, []), obj.merge(r.literal("foo")));
		@:await assertError(err("ReqlQueryLogicError", errmsg, []), obj.merge({ "foo" : r.literal(r.literal("foo")) }));
		var o = r.expr({ "a" : { "b" : 1, "c" : 2 }, "d" : 3 });
		@:await assertAtom(({ "a" : { "b" : 1, "c" : 2 }, "d" : 3, "e" : 4, "f" : 5 }), o.merge({ "e" : 4 }, { "f" : 5 }));
		@:await assertAtom(([{ "a" : { "b" : 1, "c" : 2 }, "d" : 3, "e" : 3 }, { "a" : { "b" : 1, "c" : 2 }, "d" : 4, "e" : 4 }]), r.expr([o, o.merge({ "d" : 4 })]).merge(function(row:Expr):Expr return { "e" : row["d"] }));
		@:await assertAtom(([{ "a" : { "b" : 1, "c" : 2 }, "d" : 3, "e" : 3 }, { "a" : { "b" : 1, "c" : 2 }, "d" : 4, "e" : 4 }]), r.expr([o, o.merge({ "d" : 4 })]).merge({ "e" : r.row["d"] }));
		@:await assertAtom(([{ "a" : { "b" : 2, "c" : 2 }, "d" : 3 }, { "a" : { "b" : 2, "c" : 2 }, "d" : 4 }]), r.expr([o, o.merge({ "d" : 4 })]).merge(function(row:Expr):Expr return { "a" : { "b" : 2 } }));
		@:await assertAtom(([{ "a" : { "b" : 2 }, "d" : 3 }, { "a" : { "b" : 2 }, "d" : 4 }]), r.expr([o, o.merge({ "d" : 4 })]).merge(function(row:Expr):Expr return { "a" : r.literal({ "b" : 2 }) }));
