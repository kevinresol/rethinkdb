...
@:await class TestObject extends TestBase {
	...
	override function test() {
		...
		@:await assertAtom(2, r.expr(([obj, obj.pluck("a", "b")] : Array<Dynamic>)).hasFields("a", "b").count());
		@:await assertAtom(1, r.expr(([obj, obj.pluck("a", "b")] : Array<Dynamic>)).hasFields("a", "c").count());
		@:await assertAtom(2, r.expr(([obj, obj.pluck("a", "e")] : Array<Dynamic>)).hasFields("a", { "e" : "f" }).count());
		...
		@:await assertError(err("ReqlQueryLogicError", errmsg, ([] : Array<Dynamic>)), r.literal("foo"));
		@:await assertError(err("ReqlQueryLogicError", errmsg, ([] : Array<Dynamic>)), obj.merge(r.literal("foo")));
