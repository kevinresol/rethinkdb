...
@:await class TestArray extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", "Invalid path argument `1`.", []), wftst.withFields(1));
		@:await assertError(err("ReqlQueryLogicError", "Cannot perform has_fields on a non-object non-sequence `1`.", []), r.expr(1).withFields());
		...
		@:await assertAtom([2, 3, 4], arr.map(function(v:Expr):Expr return v + 1));
		...
		@:await assertAtom(12, arr.union(arr).reduce(function(a:Expr, b:Expr):Expr return a + b));
		@:await assertAtom([1, 2, 1, 2, 1, 2], arr.concatMap(function(v:Expr):Expr return [1, 2]));
		@:await assertAtom([{ "v" : 1 }, { "v2" : 2 }, { "v" : 2 }, { "v2" : 3 }, { "v" : 3 }, { "v2" : 4 }], arr.concatMap(function(v:Expr):Expr return [{ "v" : v }, { "v2" : v + 1 }]));
		...
		@:await assertError(err("ReqlQueryLogicError", "Cannot perform bracket on a non-object non-sequence `\"a\"`.", []), r.expr([{ "a" : 1 }, "a", { "b" : 2 }, { "a" : 3, "c" : 4 }])["a"]);
		return Noise;
	}
}