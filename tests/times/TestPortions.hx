...
@:await class TestPortions extends TestBase {
	...
	override function test() {
		...
		@:await assertAtom(([1375142400, 1375142400, 1375142400, 2375136000]), ts.map(function(x:Expr):Expr return x.date()).map(function(x:Expr):Expr return x.toEpochTime()));
		@:await assertAtom(([0, 0, 0, 0]), ts.map(function(x:Expr):Expr return x.date().timeOfDay()));
		@:await assertAtom(([4896.681, 4896.682, 4897.681, 11296.681]), ts.map(function(x:Expr):Expr return x.timeOfDay()));
		@:await assertAtom(([[2013, 7, 30, 1, 21, 36.681], [2013, 7, 30, 1, 21, 36.682], [2013, 7, 30, 1, 21, 37.681], [2045, 4, 7, 3, 8, 16.681]]), ts.map(function(x:Expr):Expr return [x.year(), x.month(), x.day(), x.hours(), x.minutes(), x.seconds()]));
