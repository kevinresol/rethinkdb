...
@:await class TestApi extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", "Got 6 arguments to TIME (expected 4 or 7).", []), r.time(2013, r.july, 29, 23, 30, 0.1).toEpochTime());
		@:await assertError(err("ReqlQueryLogicError", "Got 6 arguments to TIME (expected 4 or 7).", []), r.time(2013, r.july, 29, 23, 30, 0.1).timezone());
		@:await assertError(err("ReqlQueryLogicError", "Got 5 arguments to TIME (expected 4 or 7).", []), r.time(2013, r.july, 29, 23, 30).toEpochTime());
		@:await assertError(err("ReqlQueryLogicError", "Expected type STRING but found NUMBER.", []), r.time(2013, r.july, 29, 23).toEpochTime());
		...
		@:await assertError(err("ReqlQueryLogicError", "ISO 8601 string has no time zone, and no default time zone was provided.", []), r.iso8601("2013-07-30T20:56:05").toIso8601());
