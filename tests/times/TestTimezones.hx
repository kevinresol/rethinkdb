...
@:await class TestTimezones extends TestBase {
	...
	override function test() {
		...
		@:await assertAtom(([["+00:00", 29], ["+00:00", 29], ["+00:00", 29]]), tutcs.map(function(x:Expr):Expr return [x.timezone(), x.day()]));
		@:await assertAtom(([["-00:59", 29], ["-01:00", 29], ["-01:01", 29]]), tms.map(function(x:Expr):Expr return [x.timezone(), x.day()]));
		@:await assertAtom(([["+00:59", 30], ["+01:00", 30], ["+01:01", 30]]), tps.map(function(x:Expr):Expr return [x.timezone(), x.day()]));
		@:await assertAtom(([0]), ts.concatMap(function(x:Expr):Expr return ts.map(function(y:Expr):Expr return x - y)).distinct());
