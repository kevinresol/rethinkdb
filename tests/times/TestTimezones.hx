...
@:await class TestTimezones extends TestBase {
	...
	override function test() {
		...
		@:await assertAtom(([["+00:00", 29], ["+00:00", 29], ["+00:00", 29]]), tutcs.map(function(x) return [x.timezone(), x.day()]));
		@:await assertAtom(([["-00:59", 29], ["-01:00", 29], ["-01:01", 29]]), tms.map(function(x) return [x.timezone(), x.day()]));
		@:await assertAtom(([["+00:59", 30], ["+01:00", 30], ["+01:01", 30]]), tps.map(function(x) return [x.timezone(), x.day()]));
