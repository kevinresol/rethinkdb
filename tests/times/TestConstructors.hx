...
@:await class TestConstructors extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", "Year `10000` out of valid ISO 8601 range [0, 9999].", []), r.epochTime(253430000000).toIso8601());
		@:await assertError(err("ReqlQueryLogicError", "Error in time logic: Year is out of valid range: 1400..10000.", []), r.epochTime(253440000000).year());
		@:await assertAtom(253440000000, r.epochTime(253440000000).toEpochTime());
		@:await assertAtom(1400, r.epochTime(-17980000000).year());
		@:await assertError(err("ReqlQueryLogicError", "Error in time logic: Year is out of valid range: 1400..10000.", []), r.epochTime(-17990000000).year());
