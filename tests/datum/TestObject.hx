...
@:await class TestObject extends TestBase {
	...
	override function test() {
		...
		@:await assertError(err("ReqlQueryLogicError", "OBJECT expects an even number of arguments (but found 3).", []), r.object("o", "d", "d"));
		@:await assertError(err("ReqlQueryLogicError", "Expected type STRING but found NUMBER.", []), r.object(1, 1));
		@:await assertError(err("ReqlQueryLogicError", "Duplicate key \"e\" in object.  (got 4 and 5 as values)", []), r.object("e", 4, "e", 5));
		@:await assertError(err("ReqlQueryLogicError", "Expected type DATUM but found DATABASE:", []), r.object("g", r.db("test")));
		return Noise;
	}
}