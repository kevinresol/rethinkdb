...
@:await class TestApi extends TestBase {
	...
	override function test() {
		...
		@:await assertBag([{ "index" : "rename-last", "ready" : true }, { "index" : "minval", "ready" : true }, { "index" : "maxval", "ready" : true }], tbl.indexWait("rename-last", "minval", "maxval").pluck("index", "ready"));
		...
		@:await assertBag([1, 2, 3], tbl.getAll(1, 2, 3, { "index" : "id" }).map(function(x) return x["id"]).coerceTo("ARRAY"));
		@:await assertBag([1, 2, 3], tbl.getAll(1, 2, 3).map(function(x) return x["id"]).coerceTo("ARRAY"));
