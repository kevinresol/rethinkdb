...
@:await class TestGrant extends TestBase {
	...
	override function test() {
		@:await assertError("ReqlOpFailedError", "Expected a boolean or nu" + "ll for `read`, got 1.", r.grant("test_user", { "read" : 1 }));
		@:await assertError("ReqlOpFailedError", "Unexpected key(s) `invalid`.", r.grant("test_user", { "invalid" : "invalid" }));
		...
		@:await assertError("ReqlOpFailedError", "The `connect` permission is only valid at the global scope.", r.db("database").grant("test_user", { "connect" : true }));
		@:await assertError("ReqlOpFailedError", "Unexpected key(s) `invalid`.", r.db("database").grant("test_user", { "invalid" : "invalid" }));
		...
		@:await assertError("ReqlOpFailedError", "The `connect` permission is only valid at the global scope.", r.db("database").table("table").grant("test_user", { "connect" : true }));
		@:await assertError("ReqlOpFailedError", "Unexpected key(s) `invalid`.", r.db("database").table("table").grant("test_user", { "invalid" : "invalid" }));
		...
		@:await assertError("ReqlOpFailedError", "The permissions of the user `admin` can\'t be modified.", r.grant("admin", { "config" : false }));
		@:await assertError("ReqlOpFailedError", "The permissions of the user `admin` can\'t be modified.", r.db("database").grant("admin", { "config" : false }));
		@:await assertError("ReqlOpFailedError", "The permissions of the user `admin` can\'t be modified.", r.db("database").table("table").grant("admin", { "config" : false }));
		...
		@:await assertError("ReqlPermissionError", "User `test_user` does not have the required `read` permission.", r.grant("test_user", { "read" : true }));
		@:await assertError("ReqlPermissionError", "User `test_user` does not have the required `read` permission.", r.grant("test_user", { "read" : true }));
