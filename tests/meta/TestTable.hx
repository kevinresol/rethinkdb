...
class TestTable extends TestBase {
	override function test() {
		...
		assertError("ReqlCompileError", "Expected between 1 and 2 arguments but found 3.", db.tableCreate("nonsense", "foo"));
		...
		assertAtom({ "ready" : 1 }, db.table("a").wait({ "wait_for" : "all_replicas_ready" }));
		...
		assertAtom({ "ready" : 1 }, db.wait({ "wait_for" : "all_replicas_ready" }));
		assertAtom("partial({\'rebalanced\':1})", db.rebalance());
		assertError("AttributeError", "\'module\' object has no attribute \'wait\'", r.wait());
		assertError("AttributeError", "\'module\' object has no attribute \'rebalance\'", r.rebalance());
		assertAtom("partial({\'tables_dropped\':1})", db.tableDrop("a"));
		...
		assertAtom({ "db" : "test", "name" : "testA" }, r.table("testA").config().pluck("db", "name"));
		...
		assertError("AttributeError", "\'module\' object has no attribute \'wait\'", r.wait({ "wait_for" : "all_replicas_ready", "timeout" : 5 }));
